...
public class WeakSealUpgrade extends MysteryUpgrade {
    public WeakSealUpgrade()
    {
        super(-250, true, true);
    }
    ...
    public void triggerSealEffect(MysterySeal c, AbstractMonster target, ArrayList<AbstractGameAction> actions) {
        if (c.sealValue > 0)
            actions.add(new ApplyPowerAction(target, AbstractDungeon.player, getWeak(target, c.sealValue), c.sealValue, true));
    }
    ...
    public void instantSealEffect(MysterySeal c, PretendMonster target, Map<AbstractMonster, PretendMonster> pretendMonsters) {
        if (target != null && c.sealValue > 0)
            pretendApplyPower(target, getWeak(target, c.sealValue), c.sealValue);
    }
